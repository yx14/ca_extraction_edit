...
elseif strcmpi(options.init_method,'HALS')
    ...
    %disp(size(Y_ds));
    %disp(size(K));
    [Ain,Cin,bin,fin] = HALS_initialization(Y_ds,K);
else
...
Ain = imresize(reshape(full(Ain), [ds(1),ds(2), size(Ain, 2)]),[d(1),d(2)]); %,prod(d)/d(1)/d(2)*sum(K)]);
...
Cin = imresize(Cin, [size(Ain, 2), Ts*tsub]);
